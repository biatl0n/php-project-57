<x-app-layout>
    <x-section>
            <div class="grid col-span-full">
                <h1 class="mb-5 dark:text-white text-5xl">{{ __('tasks.create-task') }}</h1>
                {{ html()->modelForm($task->name, 'POST', route('tasks.store'))->class('w-50')->open() }}
                    ...
                    <x-div>
                        <x-select-multy id="labels[]" name="labels[]" multiple :items="$labels" :selected="old('labels', $task->labels->pluck('id')->toArray())" />
